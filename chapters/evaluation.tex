...
We describe the results of these benchmarks for different sets of queues. We first compare the FT-Queue1 and FT-Queue2, then a set of high-concurrency, non-transactional queues, and lastly different variants of the flat-combining queue (transactional and non-transactional). The latter two sets are measured along with our FT-Queue2 as a baseline reference.
...
\begin{itemize}
    ...
    \item FT-FCQueue: the fully-transactional flat-combining queue.
...
\subsubsection{FT-Queue1 vs. FT-Queue2}

\floatstyle{plain} 
\restylefloat{figure}
\begin{figure}[ht!]
\caption{FT-Queue1 vs. FT-Queue2}
    \centering
    \begin{tabular}{|c|c|}
        \hline&\\
...
The performance results of FT-Queue1 and FT-Queue2 (Figure~\ref{fig:stoqueues}) can be compared to measure the effectiveness of a pessimistic approach to the pop operation. We use the better performing queue, the FT-Queue2, as the baseline reference in all future benchmarks.
...
\subsubsection{FT-FCQueue Performance}
We compare the FT-FCQueue against the NT-FCQueueWrapped and the FT-Queue2 to measure how the flat combining transactional approach described in Section~\ref{fcqueuet} performs.
Results are shown in Figure~\ref{fig:tfcqueues}.

\floatstyle{plain} 
\restylefloat{figure}
\begin{figure}[ht!]
\caption{FT-FCQueue Performance}
    \centering
    \begin{tabular}{|c|c|}
        \hline&\\
        Speed (ops/s) & Aborts (\% Transactions)\\
...
In the Push-Pop Test, the FT-Queue2 outperforms both flat combining variants, an unsurprising result given our results from the concurrent queues benchmark in Figure~\ref{fig:ntqueues}. We also note that only the FT-Queue2 experiences aborts (at approximately 1.2\% abort rates). We hypothesize this is due to a ``no-starvation'' aspect of the flat combining algorithm: the FT-Queue2's pop or push operations acquire a global queue lock. This means that the push-only or pop-only thread may continuously succeed in acquiring the lock, leading to a large sequence of pops or pushes. This can lead to the queue reaching an empty state more often, which is the only state that can cause commit-time checks to fail and the transaction to abort. The flat combining algorithm, however, applies the operations of both requesting threads during one combiner pass. Since one thread only pushes and the other only pops, no thread aborts due to seeing a marked pop, and the queue rarely reaches an empty state since both one push and one pop are applied during each pass.

The Multi-Threaded Singletons Test shows the FT-Queue2 performing approximately 2$\times$ better than the FT-FCQueue, regardless of initial queue size. Both queues do not scale, and the performance ratio remains constant regardless of the number of threads. The FT-FCQueue also experiences abort rates 1.5-2$\times$ of that of the FT-Queue2 as the number of threads increase.

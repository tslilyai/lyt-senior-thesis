...
Fullness seems to have a drastic impact on T-Chaining's performance regardless of the number of buckets in the hashmap (performance drops by approximately 50\%) and almost no impact on the performance of T-CuckooKF (Figures~\ref{fig:hm_90_1},~\ref{fig:hm_90_2}, and~\ref{fig:hm_90_3}). This is expected, because of the increased difference in the number of cache misses when T-Chaining has to follow pointers in a long chain during a lookup. 
...
We observe a similar phenomenon when the map has 125K buckets (Figure~\ref{fig:hm_90_2}) and 1M buckets (Figure~\ref{fig:hm_90_3}): T-CuckooKF's performance goes from approximately $2\times$ the performance of T-Chaining at a fullness of 5, to over $3\times$ at a fullness of 15.
...
We note that our results for both tests indicate that number of cache misses is strongly correlated with performance: T-CuckooIE performs worse than T-CuckooKF on all benchmarks, although it still performs at least as well as T-Chaining. 
...
\vspace{12pt}
\noindent\fbox{\begin{minipage}{\textwidth}
    \textbf{SUPPORTED}: The transactional cuckoo hashmaps outperform the transactional chaining hashmap by a greater margin as maximum fullness increases.
\end{minipage}}

\subsubsection{Hypothesis 4: Performance is not heavily affected by abort rate.}
    ...
    We pose this hypothesis to determine if the bottleneck in performance is the algorithm's cache usage or its abort rate. Our results in Table~\ref{tab:hm_aborts} and in Appendix~\ref{app:hm_aborts} indicate that the abort rate remains relatively constant regardless of the fullness of the hashmap, but decreases as the number of buckets in the map increases. A greater number of buckets decreases the abort rates, since the probability that two threads will simultaneously attempt to read/modify the same bucket decreases. Abort rate is negligible in nearly all tests, with the chaining hashmap experiencing the highest abort rates (the median of 5 trials never exceeding 0.006\%).
...
\vspace{12pt}
\noindent\fbox{\begin{minipage}{\textwidth}
    \textbf{SUPPORTED}: Abort rate does not heavily affect performance.
\end{minipage}}

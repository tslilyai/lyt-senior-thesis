...
\begin{itemize}
    ...
    \item Capacity: the number of buckets of the hashmap and the number of items per bucket (adjustable for CuckooHashMaps only). This, along with the maximum key value, determines the load on the hashmap. It also affects the number of cache lines accessed during the test (for example, a larger hashmap may not be expected to fit into the L2 cache, whereas a small hashmap at full capacity will fit entirely in cache). During all tests, the size of the hashmap is not allowed to outgrow its capacity.
\end{itemize}
...
\subsubsection{Tests}
\begin{itemize}
\item Multi-Thread Single-Operation Txns Test. This test is run with different numbers of threads and with different proportions of finds/inserts/deletes. 
\item Variable-Capacity Single-Operation Txns Test. This test is run with a probability of 5\% insert, 5\% delete, and 90\% find. The number of threads is fixed as 8. The only parameter changed in this test is the capacity, with the expected final size being 50\% capacity.
\end{itemize}

\subsection{Results}

We measure all hashmap's performance in terms of operations per second and cache performance (number of cache misses).

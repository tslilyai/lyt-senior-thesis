...
We first consider the results from the 33\% finds/inserts/erases test. Our results show that T-CuckooKF achieves performance $1.5$--$2\times$ that of T-Chaining as maximum fullness increases to 15, regardless of the number of buckets (Figure~\ref{fig:hm_33_2}). 
...
We observe a similar phenomenon when the map has 125K buckets and 1M buckets (Figure~\ref{fig:hm_90_2}): T-CuckooKF's performance goes from approximately $2\times$ the performance of T-Chaining at a fullness of 5, to over $3\times$ at a fullness of 15.
...
In this section, we discuss the commutativity of non-transactional and transactional hashmap operations, and argue that the additional machinery needed to synchronize of non-commutative transactions does not cripple the scalability of the chaining or cuckoo hashmap algorithms. This is because there are few added commutativity constraints for a hashmap in a transactional setting.
...
By the scalable commutativity rule, there exists a implementation of a hashmap algorithm that scales whenever insert or erase operations occur in separate buckets. Indeed, common implementations of non-transactional chaining and cuckoo algorithms will scale given most hashmap workloads and a large enough map.
...
In a transactional setting, we have the added constraint that no transaction $T1$ commutes with another transaction $T2$ if exchanging the order of $T1$ and $T2$ means that (a) any operation in one transaction observes that a key is present given one order, but not the other; and/or (b) the presence of any key in the map immediately after both transactions have committed in the history is different in one order from the resulting size in the other.
   
As we did with queue transactions, we can extend either $T1$ or $T2$ to create a transaction that does not commute with the other. WLOG, choose $T1$ to extend with ordered sets of operations, and let $O_1$ be the operations performed by $T1$. We choose $O_{app}$ to append to $O_1$ and $O_{cons}$ to add to the front of $O_1$. $O_{app}$ and $O_{cons}$ may be empty, and satisfy the properties that the new $T1$ and $T2$ do not commute: one or both of (a) and (b) must hold for $T1 = O_{cons} + O_1 + O_{app}$ and $T2$.

Again, it is simple to show that this extension constructs all possible non-commutative pairs of transactions, as was done in Section~\ref{com_scale}; we omit the equivalent explanation here. Examples of larger transactions that fail to commute are shown in Table~\ref{tab:txnal_hm_commute}.
...
\hline    
    \end{tabular}
    \caption[Example hashmap transactions that do not commute]{Example hashmap transactions that do not commute. H is the original history; H' is the history with the order of the two operations exchanged.}
    \label{tab:txnal_hm_commute}
    \end{table}

To synchronize transactions that do not commute, both the cuckoo and chaining hashmaps must synchronize all buckets modified by both transactions. We note that the modifications necessary to add transactional guarantees to the cuckoo hashmap are essentially equivalent to those necessary to add transactional guarantees to the chaining hashmap: an operation that requires synchronizing one bucket in the chaining hashmap requires synchronizing two buckets in the cuckoo hashmap. The method in which transactions are synchronized is as follows:

A transactional find in a cuckoo hashmap and a chaining hashmap adds to the read set an object representing the bucket(s) in which the object should be (or is) located. A transactional insert adds a read---and write, depending if the inserted key is already present---of the bucket(s) in which the object should be located. A transactional erase adds a read---and write, depending if the key to erase is present---of the bucket(s) in which the object should be located. At commit time, these buckets are locked. Only reads and writes that occurred within these buckets should be in the read- and write-set of the transaction. The reads are then checked, and the writes installed.

This algorithm allows for a scalable implementation of a transactional hashmap. We note that only the same buckets that are locked during the \emph{execution} of the hashmap's find or erase operations are locked again to \emph{commit} the hashmap's find or erase operation. This means that the granularity of synchronization of the transactional hashmap is still per-bucket synchronization. Adding transactions to either the cuckoo or chaining hashmap means that the buckets must be synchronized twice (once at execution, and once at commit time). 

As before, if the hashmap is large and if the workload uses a large range of keys, it is very likely that two transactions will commute because they will modify or read separate buckets. 
We reason about commutativity in a transactional setting in terms of \emph{sets} of buckets that overlap between two transactions, rather than individual buckets that are both modified/read by two operations. However, the intersection between the set of buckets modified in one transaction, and the set of buckets modified in the other, is likely to be empty or small (depending on the length of the transactions and the particular workload). We can therefore expect that an implementation of a cuckoo or chaining hashmap will still scale in a transactional setting in nearly all the same scenarios in which individual operations commute. 

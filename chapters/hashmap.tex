...
Although a non-allocating transactional cuckoo hashmap is more complex, we build it with the purpose to improve cache performance during lookups: the algorithm does not need to follow any pointers. Nonetheless, we may not see a great improvement in cache usage because we now must execute approximately double the number of lookups: each find requires performing two lookups (one during execution, and another during commit to check if the element version is still valid).
...
\subsubsection{Hypothesis 1: The non-allocating transactional cuckoo hashmap achieves the best cache usage out of all transactional hashmap.}
\label{section:hmcm}
...
The T-Chaining and T-CuckooIE hashmaps experienced a decrease in cache performance in these scenarios because a find will not need to follow pointers in the buckets of either T-CuckooKF or T-CuckooNA, whereas every find in the T-Chaining and T-CuckooIE requires following pointers. As the maximum fullness increases, the T-CuckooIE and T-Chaining maps follow an increasing number of pointers per find.
We conjecture that the T-CuckooKF has fewer cache misses than the T-CuckooNA because the additional memory accesses from following pointers is minimal (a present find follows only one pointer), and the additional lookups in the T-CuckooNA mean that the T-CuckooNA accesses approximately twice the number of buckets per operation compared to the T-CuckooKF\footnote{Our preliminary performance results for the T-CuckooNA demonstrate a similar trend, in which it performs better than the T-CuckooIE and worse than the T-CuckooNA. We omit these results because the correctness of the T-CuckooNA implementation has not been confirmed.}.

\vspace{12pt}
\noindent\fbox{\begin{minipage}{\textwidth}
    \textbf{NOT SUPPORTED}: The key-fragments transactional cuckoo hashmap achieves the best cache performance, with the non-allocating cuckoo hashmap in second place.
\end{minipage}}
...
To evaluate this hypothesis, we look at our results for a map with 10K buckets with a low maximum fullness of 5, which fits entirely in cache (Figure~\ref{hm_5}).
For all hashmaps, performance with 90\% finds (5\% inserts/erases) is nearly $0.5\times$ that of the 33\% finds/inserts/erases. This is expected: a find is both faster to perform than an insert or delete, and a find requires only reading a bucket version for all transactional hashmaps.
On both benchmarks, the cuckoo hashmaps achieve performance approximately $1.2\times$ that of T-Chaining. There is little difference between the performance of T-CuckooIE and the performance of T-CuckooKF, because cache performance does not play a significant role in these benchmarks.
...
\subsubsection{Hypothesis 3: As maximum fullness increases, the transactional cuckoo hashmaps outperform the transactional chaining hashmap by a greater margin.}

To evaluate this hypothesis, we look at our results for a map with all sizes of buckets with a high maximum fullness of 5 compared with a maximum fullness of 15.

We first consider the results from the 33\% finds/inserts/erases test. Our results show that the transactional cuckoo hashmaps, in particular T-CuckooKF, achieves performance $1.5$-$2\times$ that of T-Chaining as maximum fullness increases to 15 and when the number of buckets is 10K or 125K (Figures~\ref{fig:hm_33_1} and~\ref{fig:hm_33_2}). This is mainly due to a decrease in performance in T-Chaining, rather than an increase in performance of T-CuckooKF, whose performance remains essentially unchanged.
When the number of buckets is 1M, we see that the transactional cuckoo hashmaps also experience a decrease in performance when maximum fullness increases, which means their performance is approximately equivalent to that of T-chaining (Figure~\ref{fig:hm_33_3}). 
In general, the performance of the cuckoo hashmaps decreases slightly (less than 20\%) as hashmap size increases from 10K to 1M buckets (Figures~\ref{fig:hm_33_1},~\ref{fig:hm_33_2}). However, the chaining hashmap experiences nearly no decrease in performance. 
This is not entirely due to the number of cache misses, because T-CuckooKF still experiences fewer cache misses than T-Chaining, as we discussed in Section~\ref{section:hmcm}.
The decrease in performance of the cuckoo hashmap when the number of buckets is large may be caused by \lyt{TODO}

Our results from the 33\% finds/inserts/erases test support our hypothesis at low (10K and 125K) sizes of the cuckoo hashmap: the difference between T-CuckooKF's performance and T-Chaining's performance increases as fullness increases. However, we do not observe this phenomenon when using hashmaps with 1M buckets.
...
\begin{figure}[H]
    ...
    \caption*{10K Buckets, Maximum Fullness 5}
        \vspace{12pt}
    \end{minipage}
   \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/15HM10K:F34,I33,E33.png}}
...
\begin{figure}[H]
    \centering
    \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/5HM125K:F34,I33,E33.png}}
    \caption*{125K Buckets, Maximum Fullness 5}
        \vspace{12pt}
    \end{minipage}
   \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/15HM125K:F34,I33,E33.png}}
    \caption*{125K Buckets, Maximum Fullness 15}
        \vspace{12pt}
    \end{minipage}
...
\begin{figure}[H]
    \centering
    \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/5HM1M:F34,I33,E33.png}}
    \caption*{1M Buckets, Maximum Fullness 5}
        \vspace{12pt}
    \end{minipage}
   \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/15HM1M:F34,I33,E33.png}}
...
Fullness seems to have a drastic impact on T-Chaining's performance with 10K buckets in the hashmap (performance drops by approximately 50\%) and almost no impact on the performance of T-CuckooKF (Figure~\ref{fig:hm_90_1}). This is expected, because of the increased difference in the number of cache misses when T-Chaining has to follow pointers in a long chain during a lookup. At a fullness of 15, T-CuckooKF performs over $2\times$ better than T-Chaining.
When using maps with 125K buckets (Figure~\ref{fig:hm_90_2}, the difference in performance of T-Chaining and T-CuckooKF is less significant, and the performance of T-Chaining decreases less as fullness increases from 5 to 15. However, we still observe that T-CuckooKF goes from approximately equivalent performance to T-Chaining at a fullness of 5, to $1.2\times$ the performance of T-Chaining at a fullness of 15.
We observe more unexpected results when using maps with 1M buckets (Figure~\ref{fig:hm_90_3}): performance of T-Chaining increases by nearly 149\% compared to its performance on the same tests using maps of 125K buckets. Furthermore, we see that the performance of the cuckoo hashmaps has decreased by approximately 33\% from the tests run with 10K buckets. Furthermore, the performance of T-Chaining does not decrease as fullness increases.
\lyt{TODO why does this occur???}

Again, we note that this result is partially consistent with the claims made about cuckoo hashmaps in Section~\ref{section:ntchm_algo}. While our hypothesis is supported for maps using 10K and 125K buckets, we note that T-Chaining with 1 million buckets appears to experience little to no decrease in performance as maximum fullness increases. Furthermore, the cuckoo hashmaps' performance is heavily affected by the large map size.\lyt{TODO}.

A find for the chaining hashmap is less expensive---does not need to iterate
...
\begin{figure}[H]
    ...
    \caption*{10K Buckets, Maximum Fullness 5}
        \vspace{12pt}
    \end{minipage}
   \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/15HM10K:F90,I5,E5.png}}
    \caption*{10K Buckets, Maximum Fullness 15}
        \vspace{12pt}
    \end{minipage}
...
\begin{figure}[H]
    \centering
    \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/5HM125K:F90,I5,E5.png}}
    \caption*{125K Buckets, Maximum Fullness 5}
        \vspace{12pt}
    \end{minipage}
   \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/15HM125K:F90,I5,E5.png}}
    \caption*{125K Buckets, Maximum Fullness 15}
        \vspace{12pt}
    \end{minipage}
...
\begin{figure}[H]
    \centering
    \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/5HM1M:F90,I5,E5.png}}
    \caption*{1M Buckets, Maximum Fullness 5}
        \vspace{12pt}
    \end{minipage}
   \begin{minipage}{0.75\textwidth}\boxed{\includegraphics[width=\textwidth]{maps/15HM1M:F90,I5,E5.png}}
    \caption*{1M Buckets, Maximum Fullness 15}
        \vspace{12pt}
    \end{minipage}
...
\vspace{12pt}
\noindent\fbox{\begin{minipage}{\textwidth}
    \textbf{PARTLY SUPPORTED}: With a small to medium number of buckets in the hashmap, the transactional cuckoo hashmaps outperform the transactional chaining hashmap by a greater margin as maximum fullness increases.
\end{minipage}}

\subsubsection{Hypothesis 3: Performance is not heavily affected by abort rate.}
